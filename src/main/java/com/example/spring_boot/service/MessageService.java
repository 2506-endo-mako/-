...
public class MessageService {
     ...
     * レコード全件取得処理
     */
    public List<MessagesForm> findAllMessages(String startDate,String endDate, String category) {
        ...
        List<Message> results = messageRepository.findByCreatedDateBetweenAndCategoryOrderByCreatedDateAsc(start,end,category);
        List<MessagesForm> Messages = setMessagesForm(results);
        return Messages;
    }
    /*
     * DBから取得したデータをFormに設定
     */
    private List<MessagesForm> setMessagesForm(List<Message> results) {
        List<MessagesForm> Messages = new ArrayList<>();
...
//            messages.setUpdatedDate(result.getUpdatedDate());
     ...
     * レコード追加
     */
    public void saveMessages(MessagesForm reqMessages) {
     ...
     */
    private Message setMessagesEntity(MessagesForm reqMessages) {
